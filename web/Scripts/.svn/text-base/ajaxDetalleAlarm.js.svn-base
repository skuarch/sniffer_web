...
*/
function ajaxDetalleAlarm(label){
	...
	var r3 = Math.round(r1+r2,2);
        ...
        var ruta = "Scripts/tablaDetalleAlarm.jsp?fecha_inicial="+fecha_inicial+"&fecha_final="+fecha_final+"&opcionGrafica="+opcionGrafica+"&r3="+r3+"&clic="+clic+"&tipoGrafica="+tipoGrafica+"&tiempoGrafica="+tiempoGrafica+"&unidadMedida="+unidadMedida+"&nombreUnidadMedida="+nombreUnidadMedida+"&label="+label+"&paginaGrafica="+paginaGrafica;
	...
	ajax.onreadystatechange=function(){
		...
		if(ajax.readyState==1){
			...
			document.getElementById('contenedor2').innerHTML = cargador;
		}//fin 1
		...
		if(ajax.readyState==4){
			...
			document.getElementById('contenedor2').innerHTML = ajax.responseText;
                        sortables_init();
